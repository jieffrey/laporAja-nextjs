import {
  RiLayoutGridFill, RiFileList3Fill, RiTeamFill,
  RiTrophyFill, RiUserFill, RiAddCircleFill, RiAccountCircleFill,
} from "react-icons/ri"
...
export type MenuItem = {
  ...
  href:  string
  Icon:  IconType
  roles?: UserRole[]   // si undefined = semua role bisa akses
}
...
export function getPageTitle(pathname: string): string {
  ...
  const prefix = all
    .filter(m => m.href !== "/admin" && m.href !== "/user" && pathname.startsWith(m.href))
